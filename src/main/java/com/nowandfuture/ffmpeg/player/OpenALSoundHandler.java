...
public class OpenALSoundHandler implements PlayHandler.SoundPlayHandler {
    ...
    public long handle(Frame frame) throws InterruptedException {
        ...
        long duration = (long) (mono.length / sampleRate * 500 / audioChannels);

        AudioFormat format = SoundUtils.getAudioFormat(sampleFormat, sampleRate, audioChannels, sampleRate);

        //+1 is unnecessary, but is not wrong! when a piece of sound is playing, the hardware may play the Nth byte
        ...
        simplePlayer.getSyncInfo().offset = - queued * duration;
        ...
        if(queued > OpenAlQueueMaxSize) return 0;
